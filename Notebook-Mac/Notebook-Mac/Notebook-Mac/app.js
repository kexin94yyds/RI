...
function updateNativeTitle() {
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.titleHandler) {
        const titleEl = document.querySelector('.navigation-title') || document.querySelector('title');
        let titleText = titleEl ? (titleEl.textContent || titleEl.innerText) : "RI 笔记";
        
        // 限制在 10 个字符以内
        const nativeTitle = titleText.substring(0, 10) + (titleText.length > 10 ? '...' : '');
        window.webkit.messageHandlers.titleHandler.postMessage(nativeTitle);
    }
}
