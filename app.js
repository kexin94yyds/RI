...
function generateModeMarkdown(mode, words, imageFiles = []) {
  ...
  words.forEach((word, index) => {
    ...
    if (itemType === 'image') {
      ...
      lines.push(`### ${index + 1}. [链接]`);
      lines.push('');
      lines.push(`[${content}](${content})`);
    } else {
      // 文本内容
      const preview = content.length > 50 ? content.substring(0, 50) + '...' : content;
      lines.push(`### ${index + 1}. ${preview.replace(/\n/g, ' ')}`);
      lines.push('');
      // 使用代码块包裹多行文本
      if (content.includes('\n')) {
        lines.push('```');
...
// 逐个下载文件（无 JSZip 时的备选方案）
async function exportFilesSequentially(files, timestamp) {
  // 提示用户将下载多个文件
  if (files.length > 1) {
    alert(`将下载 ${files.length} 个 Markdown 文件，请在浏览器中允许多文件下载。`);
  }
