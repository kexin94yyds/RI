...
- 初次修复：2025-11-01
- 彻底修复：2025-11-07（修复了 IPC 事件处理和数据库加载逻辑）
...
**修复方案**：
- `modes-sync` 现在只更新模式列表和元数据，不会重新加载笔记内容
- `mode-changed` 专门负责真正的模式切换，并从数据库加载完整数据
...
**修复方案**：
- 所有模式切换现在都调用 `await getMode(modeId)` 从数据库重新加载
- 确保每次切换都使用最新的数据，而不是主窗口传来的可能过期的数据
